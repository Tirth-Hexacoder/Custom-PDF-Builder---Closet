...
export type ProjectedObjectCropTimingResult = {
  image: string;
  processingMs: number;
  encodeMs: number;
  elapsedMs: number;
...
export function captureCanvasWithProjectedObjectCropTimed({
  sourceCanvas,
  camera,
  target,
  paddingPx = 8,
  ...
  if (!rect) {
    ...
    const image = getFallbackDataUrl(sourceCanvas, mimeType, quality);
    const encodeMs = performance.now() - encodeStart;
    const processingMs = performance.now() - start - encodeMs;
